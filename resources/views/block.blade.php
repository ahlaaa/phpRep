...
<!--                            {{--{!! $productClass->where('status',3)->count('id') !!}--}}-->
<!--                        </span>-->
<!--                    </p>-->
...
<!--                        <span class="col-sm-6 clearfix">-->
<!--                            {{--{!! $orderClass->where('status',2)->count('id') !!}--}}-->
<!--                        </span>-->
<!--                    </p>-->
...
<!--                        <span class="col-sm-6 clearfix">-->
<!--                            {{--{!! $maintainClass->whereIn('status',[1,2,3])->count('id') !!}--}}-->
<!--                        </span>-->
<!--                    </p>-->
    ...
    <div class="row box-body">
        <div class="col-sm-6 panel-body" style="min-height: 430px;">
                ...
                <span class="col-sm-7 pull-right">
                    <div class="row">
                        <span class="col-sm-3" style="max-height: 31px;">
                            <span><a class="{{ (Request::is('home*')||Request::is('admin*'))&&request()->get('type',1)==1?'unclick':'' }}"
                                     href="{!! url('home?type=1') !!}">今日</a></span>
                            <br>
                            <span class="{{ (Request::is('home*')||Request::is('admin*'))&&request()->get('type',1)==1?'btmc':'' }}"></span>
                        </span>
                        <span class="col-sm-3">
                            <span><a class="{{ (Request::is('home*')||Request::is('admin*'))&&request()->get('type',1)==-1?'unclick':'' }}"
                                     href="{!! url('home?type=-1') !!}">昨日</a></span>
                            <br>
                            <span class="{{ (Request::is('home*')||Request::is('admin*'))&&request()->get('type',1)==-1?'btmc':'' }}"></span>
                        </span>
                        <span class="col-sm-3">
                            <span><a class="{{ (Request::is('home*')||Request::is('admin*'))&&request()->get('type',1)==7?'unclick':'' }}"
                                     href="{!! url('home?type=7') !!}">最近7日</a></span>
                            <br>
                            <span class="{{ (Request::is('home*')||Request::is('admin*'))&&request()->get('type',1)==7?'btmc':'' }}"></span>
                        </span>
                        <span class="col-sm-3">
                            <span><a class="{{ (Request::is('home*')||Request::is('admin*'))&&request()->get('type',1)==31?'unclick':'' }}"
                                     href="{!! url('home?type=31') !!}">本月</a></span>
                            <br>
                            <span class="{{ (Request::is('home*')||Request::is('admin*'))&&request()->get('type',1)==31?'btmc':'' }}"></span>
                        </span>
                    </div>
                </span>
            </div>
            <div class="row" style="margin-top: 27%;text-align: center;">
                ...
                ?>
                <div class="col-sm-4">
                    <p style="color: #f9a20a;font-size: 25px;">{!! $deal_num.'/'.$all_nums !!}</p>
                    <p>成交量/交易量(件)</p>
                </div>
                <div class="col-sm-4">
                    <p style="color: #f9a20a;font-size: 25px;">{!! round($deal_price,2) !!}/{!! round($orders->sum('amount'),2) !!}</p>
                    <p>成交额/交易额(元)</p>
                </div>
                <div class="col-sm-4">
                    <p style="color: #f9a20a;font-size: 25px;">{!! round(($orders->sum('amount'))/(sizeof($uLists)),2) !!}</p>
                    <p>人均消费(元)</p>
                </div>
            </div>
        </div>
        <div class="col-sm-5 panel-body" style="margin-left: 10px;min-height: 430px;">
            ...
            <div class="row panel-body">
                <table class="table table-responsive">
                    ...
                    <tr>
                        <td>{!! $loop->index+1 !!}</td>
                        <td class="spehidden">{!! $product->name !!}</td>
                        <td>{!! $product->sort !!}</td>
                        <td>{!! round($product->price*$product->sort,2) !!}</td>
                    </tr>
                    @endforeach
                    </tbody>
                </table>
...
<script>
    var list = [];
    @foreach($chartList as $k=>$list)
    var k = ''+{!! json_encode($k) !!},list1 = {!! json_encode($list1[$k]??[]) !!},list2 = {!! json_encode($list2[$k]??[]) !!};
    var datas = {
        'period': k,
        'num1': list1.num1,//交易量
        'num2': list2.num2,//成交量
