<html>
                    ...
                    <header class="main-header" style="background-color: white;">
                        ...
                        <div class="col-sm-1 user-panel"
                            ...
                            <div class="pull-left image" style="width: 30px;">
                                <img src="{{ Auth::user()->img_head }}" class="img-circle" style="margin-left: 10px;"
                                     alt="User Image"/>
                            </div>
                                    ...
                                    <li class="dropdown user user-menu">
                                        ...
                                        <ul class="dropdown-menu">
                                            ...
                                            <li class="user-header">
                                                ...
                                                <p>
                                                    {!! Auth::user()->name !!}
                                                    <small>注册时期 {!! Auth::user()->created_at->format('Y-m-d') !!}</small>
                                                </p>
                                            </li>
                                            <!-- Menu Footer-->
	...
	<div id="bottom-box">
		...
		<div style="padding:5px 10px;"><a href="{!! url('orders?search=2&searchFields=status:=') !!}">您有 <span style="color:red;">
        <?php
            echo app(\App\Models\Order::class)->where("status",2)->count();
        ?>
        </span> 笔订单未处理</a></div>
		<div style="padding:5px 10px;"><a href="{!! url('stores?search=1&searchFields=status:=') !!}">您有 <span style="color:red;">
        <?php
            echo app(\App\Models\Store::class)->where("status",1)->count();
        ?>
        </span> 笔门店申请未处理</a></div>
		<div style="padding:5px 10px;"><a href="{!! url('users?type=1&search=1&searchFields=status:=') !!}">您有 <span style="color:red;">
        <?php
            echo app(\App\Models\User::class)->where([["status",1],["type",1]])->count();
        ?>
        </span> 笔代理商申请未处理</a></div>
		<div style="padding:5px 10px;"><a href="{!! url('withdraws?search=1&searchFields=status:=') !!}">您有 <span style="color:red;">
        <?php
            echo app(\App\Models\Withdraw::class)->where("status",1)->count();
        ?>
        </span> 笔提现申请未处理</a></div>
		<div style="padding:5px 10px;"><a href="{!! url('war4bala') !!}">近期有 <span style="color:red;">
        <?php
