...
<div class="form-group col-sm-12">
    ...
    <div class="col-sm-8" id="content_div" style="width:100%; height:200px;"><?php echo optional($route)->remark??''; ?></div>
...
<div class="col-sm-12">
    ...
    <div class="col-sm-12" style="margin-left: 25px;background-color: #ccc;padding: 10px 10px;min-height: 75%;">
        <table class="table form-group">
            ...
            <tbody id="parameters">
            <?php if(!isset($route->parameters) | 0 == sizeof($route->parameters??[])): ?>
            <tr class="count_row">
            ...
            <?php else: ?>
            <?php $__currentLoopData = $route->parameters; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $parameter): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
            <tr class="count_row">
                <td><input value="<?php echo $parameter->pivot->name; ?>" name="parameters[<?php echo $parameter->id; ?>][name]" type="text" class="form-control"/></td>
                <td><input value="<?php echo $parameter->pivot->remark; ?>" name="parameters[<?php echo $parameter->id; ?>][remark]" type="text" class="form-control"/></td>
                <td>
                    <label class="controls col-sm-10">
                        <input id="p<?php echo $loop->index+1; ?>_images_file" type="file" class="file-loading" accept="image/*">
                        <input type="hidden" name="parameters[<?php echo $parameter->id; ?>][images]" id="p<?php echo $loop->index+1; ?>_images"
                               value="<?php echo e($parameter->pivot->images ?? ''); ?>">
                    </label>
                    <!--                    <input value="<?php echo $parameter->pivot->images; ?>" name="routes[<?php echo $parameter->parameter_id; ?>][images]" type="text" class="form-control"/>-->
                </td>
                <td><input type="button" class="btn btn-danger" onclick="javascript:remove_parameters(this);"
                           value="删除"/></td>
            </tr>
...
<div class="col-sm-12">
    <div class="form-group col-sm-12 pull-left" style="padding: 0px;">
        <?php echo Form::label('images','旅游路线总图',['class'=>'col-sm-3']); ?>

        <div class="controls col-sm-10">
            <input id="images_file" type="file" class="file-loading" accept="image/*">
            <input type="hidden" name="images" id="images"
                   value="<?php echo e(optional($route)->images ?? ''); ?>">
            <?php  ?>
        </div>
...
<div class="form-group col-sm-12">
    ...
    <a href="<?php echo route('routes.index'); ?>" class="btn btn-default">返回</a>
</div>
