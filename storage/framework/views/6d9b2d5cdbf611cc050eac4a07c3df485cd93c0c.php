<table class="table table-responsive" id="grades-table">
    <thead>
        <tr>
            ...
            <td colspan="5">
                <button type="button" onclick="javascript:status_change(this);" class="col-sm-3 btn btn-default" data-id="1">下架</button>
                <button data-id="2" onclick="javascript:status_change(this);" class="col-sm-3 btn btn-default" style="margin: 0px 16px;">删除</button>
<!--                <button class="col-sm-3 btn btn-default">批量分类</button>-->
            </td>
        </tr>
        <tr>
    ...
    <?php $__currentLoopData = $products; $__env->addLoop($__currentLoopData); foreach($__currentLoopData as $v): $__env->incrementLoopIndices(); $loop = $__env->getLastLoop(); ?>
        <tr>
            ...
            <td style="text-align:center" width="8%" height="50px"><?php echo $v->status_str; ?></td>
            <td style="text-align:center" width="10%" height="50px">
                ...
                <div class='btn-group' style="display: flex;">
                    ...
                    <a href="<?php echo url('products.untrash/'.$v->id.'?uri='.request()->url()); ?>" class="btn btn-primary btn-sm">撤回</a>
                </div>
                <?php echo Form::close(); ?>
...
<script>
    ...
    function status_change(e){
        if ($("#grades-table thead input[type='checkbox']").eq(0).prop('checked')){
            $("#status_change").html("");
            if(1===$(e).data('id')) {
                $("#status_change").append('<input type="text" value="2" name="change_type"/>');
